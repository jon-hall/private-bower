require('./utils');
var q = require('q');
var path = require('path');
var logger = require('./logger');
var mkdirp = require('mkdirp');
var exec = require('child_process').exec;

module.exports = function RepoCache(options) {
    _init();
    function _init() {
        return _createDirectory(options.repoCacheRoot)
            .then(_checkGitInstalled)
            .then(_startGitDaemon)
            .fail(function (err) {
                logger.log('Failed to initialize public repository cache'.red);
                process.nextTick(function () {
                    throw new Error(err);
                });
            });
    }

    function _getCachedRepo(repoName, repoUrl, lastUpdated) {
        var deferred = q.defer();

        // Check if it's already cached
        var cacheRepoAddr = 'git://{0}:{1}/{2}'.format(options.hostName, options.port, repoName);
        var checkRepoCommand = 'git ls-remote {0}'.format(cacheRepoAddr);

        exec(checkRepoCommand, function (error) {
            if (error) {
                // Repo not cached - we need to clone and return the result
                _cacheRepo(repoName, repoUrl).then(function () {
                    return {
                        name: repoName,
                        repo: cacheRepoAddr,
                        requiresRegistration: true,
                        lastUpdated: (new Date()).getTime()
                    };
                }).then(deferred.resolve, deferred.reject);

                return;
            }

            // Are we going to update the repo...
            if(options.minHoursBetweenUpdates !== false) {
                // When was this package last updated, if no timestamp supplied we assume never
                var timeSinceLastUpdate = !lastUpdated ? Infinity : (new Date()).getTime() - lastUpdated;

                if(timeSinceLastUpdate > (options.minHoursBetweenUpdates * 60 * 60 * 1000)) {
                    // Repo was last updated longer ago than our threshold, so git pull then return the repo info
                    _updateCachedRepo(repoName).then(function (timestamp) {
                        return {
                            name: repoName,
                            repo: cacheRepoAddr,
                            lastUpdated: timestamp
                        };
                    }).then(deferred.resolve, deferred.reject);
                    return;
                }
            }

            // We aren't updating, so just send back the info immediately
            deferred.resolve({
                name: repoName,
                repo: cacheRepoAddr,
                lastUpdated: lastUpdated
            });
        });

        return deferred.promise;
    }

    function _cacheRepo(repoName, repoUrl) {
        var deferred = q.defer();

        var packageDirectory = path.join(options.repoCacheRoot, repoName);

        _createDirectory(packageDirectory)
            .then(function () {
                _cloneGitRepo(repoUrl, packageDirectory, repoName).then(deferred.resolve, deferred.reject);
            }, function () {
                logger.log('Failed to clone '.red + repoUrl);
                deferred.reject();
            });

        return deferred.promise;
    }

    function _checkGitInstalled() {
        var deferred = q.defer();

        exec('git daemon --version', function (error, stdout, stderr) {
            // git-daemon is often installed separately from git.
            switch (error && error.code) {
                case 129: // If git && git-daemon is installed, the error code will be 129.
                    deferred.resolve();
                    break;
                case 127: // If git isn't installed, the error code will be 127.
                    logger.log('Git must be installed and added to path'.red);
                    deferred.reject(stderr);
                    break;
                case 1: // If git is installed but git-daemon is not, the error code will be 1.
                    logger.log('Git-daemon must be installed'.red);
                    deferred.reject(stderr);
                    break;
                default: // This shouldn't happen. The command passed and error is null, or some other error has occurred.
                    logger.log('Unknown error verifying git dependencies'.red);
                    deferred.reject(stderr);
                    break;
            }
        });

        return deferred.promise;
    }

    function _createDirectory(dir) {
        var deferred = q.defer();

        mkdirp(dir, function (err) {
            if (err) {
                deferred.reject();
                return;
            }

            deferred.resolve();
        });

        return deferred.promise;
    }

    function _startGitDaemon() {
        var deferred = q.defer();

        process.chdir(options.repoCacheRoot);

        var gitCommand = 'git daemon --reuseaddr --verbose --base-path="{0}" --listen={1} --port={2} --export-all'
            .format(options.repoCacheRoot, options.hostName, options.port);

        logger.log('Starting git cache server');

        exec(gitCommand, function (error, stdout, stderr) {
            if (error) {
                deferred.reject(stderr);
                return;
            }

            deferred.resolve();
        });

        return deferred.promise;
    }

    function _cloneGitRepo(repoUrl, packageDirectory, repoName) {
        var deferred = q.defer();

        process.chdir(packageDirectory);

        var gitCommand = 'git clone {0} ./'.format(repoUrl);

        logger.log('Cloning {0} ...'.format(repoName));

        exec(gitCommand, function (error, stdout, stderr) {
            if (error) {
                deferred.reject(stderr);
                return;
            }

            logger.log('Cloned {0} git repository to private'.format(repoName));

            deferred.resolve((new Date()).getTime());
        });

        return deferred.promise;
    }

    function _updateCachedRepo(repoName) {
        var deferred = q.defer();

        var packageDirectory = path.join(options.repoCacheRoot, repoName);

        process.chdir(packageDirectory);

        var gitCommand = 'git pull';

        logger.log('Updating {0} ...'.format(repoName));

        exec(gitCommand, function (error, stdout, stderr) {
            if (error) {
                deferred.reject(stderr);
                return;
            }

            logger.log('Updated {0} git repository to latest'.format(repoName));

            // Resolve with a new 'lastUpdated' timestamp since we pulled successfully
            deferred.resolve((new Date()).getTime());
        });

        return deferred.promise;
    }

    return {
        getCachedRepo: _getCachedRepo
    };
};