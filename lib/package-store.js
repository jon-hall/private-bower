...
module.exports = function PackageStore(options) {
    ...
    function _registerPackages(register, isPrivate) {
        for(var i = 0, len = register.length; i < len; i++) {
            ...
            _packages[registerPackage.name] = {
                ...
                hits: 0,
                private: isPrivate || registerPackage.private,
                lastUpdated: registerPackage.lastUpdated
            };
    ...
    function _setPackageLastUpdated(packageName, lastUpdated) {
        if(_packages[packageName]) {
            _packages[packageName].lastUpdated = lastUpdated;
            _persistPackages();
        }
    }
    ...
    return {
        ...
        setPackageLastUpdated: _setPackageLastUpdated
    };
};